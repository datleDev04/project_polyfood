...
<main class="w-full px-8 mt-10 sm:px-24 md:px-32 lg:px-36 font-montserrat">
  ...
  <form action="?url=payment" method="post" class="flex flex-col gap-3 mx-auto ">
...
border: 1px solid rgba( 255, 255, 255, 0.18 );" type="email" name="email" id="email" class="w-full p-3 text-sm form__input focus:outline-none" placeholder="Nhập email" value="<?= isset($_SESSION['user']) ? $email : '' ?>" />
      ...
      ?>
        <div style="
          ...
          <div class="bg-white rounded product__img">
            <img style="width:96px; height: 96px;" src="src/assets/images/products/<?= $item['image'] ?>" alt="" class="object-cover rounded-md " />
          </div>
          <div class="flex flex-col justify-between product__name">
            ...
            <p class="text-sm text-gray-500">
              Đơn giá :
              <span class="text-orange-500"> <?= number_format(($item['price']* (1 - $item['discount'] / 100)), 0, ",", ".") ?> đ</span>
            </p>
            <p class="text-sm text-gray-500">
              Thành tiền :
              <span class="text-orange-500">
    ...
    <a href="?url=allproduct" class="flex text-sm font-semibold text-indigo-600">
